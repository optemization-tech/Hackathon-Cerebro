...
export function buildPreamble(row: DocsRow): string {
	...
	lines.push("");
	lines.push(
		"This record was ingested from the Optemization Docs Database by the documents-ingest worker.",
	);
	lines.push(
		"Structured fields on the Notion page above (Source, Data Type, Person Source, ID) describe ingestion metadata; the body below is the document content.",
	);
	lines.push("");
